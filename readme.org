...
~rv32-sail~ does not just come with the emulator: it also comes with a handy
toolkit and environment containing a GCC cross compiler, the emulator, as well
as a robust build system and test suite. If you want to try RISC-V assembly
programming, write your own simple "bare metal" programs, or extend the RISC-V
instruction set, this might be for you!
...
- High-level RISC-V ISA emulator, written in Sail
  ...
  - Compiled to C for an executable emulator.
  - The RISC-V instruction decoder is automatically generated from an
    even higher-level spec describing the decoding layout for each
    instruction, written in Haskell.
- Robust build system, toolchain, programming environment.
  ...
  - The build system is written in Haskell, using [[https://shakebuild.com][Shake]] -- it's fast
...
- Included toolchain for C programming
...
are written, etc. Should you ignore this, I cannot help you, and you should not
...
This will take some time to build the GCC cross compiler for RISC-V, as well as
the Sail toolchain. Afterwords, it will compile the emulator as well as some
...
Once you're inside ~nix-shell~, you can also run the ~bake~ command, which is a
simple wrapper to run the ~bake.hs~ script, with all the same arguments:
...
The main difference between running ~bake~ inside ~nix-shell~ or running
~bake.hs~ directly is one of speed: the ~bake.hs~ script uses a "shell
scripting" feature of Nix, which means invoking the script causes Nix to run. It
takes a second or two to set up the build environment, and then run the script,
every time you invoke it. (Despite the slowdown, the advantage of running
~bake.hs~ directly without ~nix-shell~ is that it essentially makes the build
system completely transparent to the user.)

In contrast, running ~bake~ inside the ~nix-shell~ just runs the script
directly, since the build environment is already available, and this means it
"starts up" faster.

For iterative development, you may find having an extra terminal or tmux window
where you run ~bake~ quite useful!
...
The self-testing firmware is available under ~./build/t/firmware.elf~, and can
be loaded immediately. At the end, the emulator will spit out some runtime
statistics, as well as a register dump:

#+BEGIN_SRC
./build/cruise.opt -e build/t/firmware.elf
...
The emulator has several configuration options which can be set at runtime
in order to control various aspects of the machine's behavior. Notably, this
includes things like disabling certain instruction set features, etc.

To get the list of options, invoke the ~cruise.opt~ executable with the
arguments ~-C help~:

#+BEGIN_SRC
./build/cruise.opt -C help
#+END_SRC

** Cleaning up

You can completely delete this directory later at any time if you want to clean
things up, or run ~./bake.hs clean~ to have it done for you.
